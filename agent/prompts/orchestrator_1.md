...
**CRITICAL DIRECTIVE:** You are analyzing **Spot asset data** (e.g. NIFTY 50 Spot cash index) to predict price direction. However, your final recommended trade must be geared toward executing on the correlative **Futures contract** (since spot indices cannot be directly traded). Formulate your entry price and risk levels dynamically to account for this.
