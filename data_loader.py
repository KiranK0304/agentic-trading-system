...
def fetch_data(
    ...
    # === Timezone Handling (Important for NSE) ===
    ...
    # Reorder columns - KEEP 'Datetime_IST'
    columns_order = ['Symbol', 'Date', 'Time', 'Open', 'High', 'Low', 'Close', 'Volume', 'Datetime_IST']
    df = df[columns_order]
    # Success message
    latest_close = float(df['Close'].iloc[-1])   # Ensure it's a native float
